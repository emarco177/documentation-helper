...
async def process_documents_async(documents: List[Document], batch_size: int = 50):
    """Process documents in batches asynchronously."""
    log_header("VECTOR STORAGE PHASE")
    log_info(
        f"📚 ChromaDB: Preparing to add {len(documents)} documents to vector store",
        Colors.DARKCYAN,
    )
    ...
    log_info(
        f"📦 ChromaDB: Split into {len(batches)} batches of {batch_size} documents each"
    )
    ...
    async def add_batch(batch: List[Document], batch_num: int):
        try:
            ...
            log_success(
                f"ChromaDB: Successfully added batch {batch_num}/{len(batches)} ({len(batch)} documents)"
            )
        except Exception as e:
            log_error(f"ChromaDB: Failed to add batch {batch_num} - {e}")
            return False
        return True
    ...
    if successful == len(batches):
        log_success(
            f"ChromaDB: All batches processed successfully! ({successful}/{len(batches)})"
        )
    else:
        log_warning(
            f"ChromaDB: Processed {successful}/{len(batches)} batches successfully"
        )
...
async def main():
    ...
    await process_documents_async(splitted_docs, batch_size=500)
    ...
    log_info(f"   • Vector store: ChromaDB at ./chroma_db")
